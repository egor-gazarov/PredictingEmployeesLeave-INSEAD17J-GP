...
Below is brief info about project dataset structure.
...
Analysis of satisfaction level shows that employees that left the company had satisfaction level at all levels (low, medium, and high). On the other hand, the analysis also confirmed that most of the employees who stayed have medium to high satisfaction level. This suggests that high satisfaction level is not a predictor of employee staying, but low satisfaction level is a stronger predictor of employees leaving.
...
Analysis of last evaluation shows that employees that left scored medium and high score during the last evaluation , while employees that stayed are distributed more evenly along the spectrum of medium-high. Strong visual differences in distributions suggest that we can expect this variable to appear as significant for our predicting model.
...
Analysis of number of projects suggests that the ideal number of project is probably 3-4 projects per employee. The bulk of employees that left the company had 2 projects only.
...
Analysis of monthly hours suggest that the ideal work intensity is between 150-250 hours. There are high concentration of employees that left with work hours lower than 150 hours or higher than 250 hours. Strong visual differences in distributions suggest that we can expect this variable to appear as significant for our predicting model.
...
The tenure time analysis of employees who left suggest that the highest risk of employees leaving occur at year 3 of employment. The company might want to do more analysis around time at company because the graph of employees that are staying indicates that most of the company's employees have only spent 2-3 years. That means a large portion of their employees might be at risk of leaving.
...
Surprisingly share of employees who had accident is higher among employees who stayed. Intuitively we could expect that employees leave company after having accident. 
...
Slightly higher share of employees who were promoted is observed among employees who stayed. This makes sense. Looks like company if generally not very active with promoting its staff.
...
The chart below shows distribution of people who left and who stayed between departments. Distributions look visually similar with some differences being observed in R&D, Accounting, HR departments. We might suspect this department might appear as predicting factors in our model.
...
The table below shows the ratio of number employees who left over the number of employees who stayed. The higher the ratio indicated that a division has a higher turnover. The table below suggested that the turnover ratio differs across the different division, with R&D having the lowest turnover and HR having the highest turnover.

```{r , echo=TRUE, message=FALSE, warning=FALSE}
leftdiv <- as.data.frame(table(HR.left$Division))
staydiv <- as.data.frame(table(HR.stayed$Division))
cbind(leftdiv[1],round(leftdiv[-1]/staydiv[-1],2))
```


### Salary

Comparing distribution of people who left and stayed across salary groups, we see that in stayed category there are relatively more people who had medium and high salaries.
...
Since predictive quality is similar on training and test datasets, we conclude that we managed to avoid overfitting.
...
We can also use variables proven to be significant for prediction as actionable recommendations for HR team:
+ to take urgent measures if employee's working hours go above 287;
+ to pay special attention to employees with satisfaction level below 0.46;
+ to focus preventive measures for employees retention on people with 3-5 years in company;
+ to monitor people with evaluations below 0.57;
+ to check reasons why employees might have too light project workload (2 project or less).
...
+ There is a high probability that Jackie will leave the company in the near future. HR can then decide whether they should take preemptive measures to prevent Jackie from leaving
...
```
In this case, we see that 1 in 10 employees (or 10% of employees) have a high probability of leaving the company in the near future. Using this data, HR can set aside enough budget to plan for back-filling these employees.
