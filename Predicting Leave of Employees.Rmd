...
source("./R/library.R")
install_load("ggplot2", "corrplot", "caret", "C50", "cwhmisc", "stringr", "gmodels", "rpart", "rattle", "rpart.plot", "RColorBrewer", "party", "partykit", "caret")
...
fit <- rpart(left ~ satisfaction_level + last_evaluation + number_project + average_montly_hours + time_spend_company + Work_accident + promotion_last_5years + Division.hr + Division.IT + Division.management + Division.marketing + Division.product_mng + Division.RandD + Division.sales + Division.support + Division.technical + salary.low + salary.medium, method="class", data=TrainDS)
prp(fit, uniform=TRUE, 
  	main="Classification Tree for Employee")
```
...
fit2 <- rpart(left ~ satisfaction_level + last_evaluation + number_project + average_montly_hours + time_spend_company, method="class", data=TrainDS)
prp(fit2, uniform=TRUE, 
  	main="Classification Tree for Employee")
...
PredictionTree <- predict(fit2, TestDS, type="class")
CT.Tree <- CrossTable(TestDS$left, PredictionTree, prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE, dnn = c('actual leave', 'predicted leave'))
CT.Tree.DF <- as.data.frame(CT.Tree)
```
